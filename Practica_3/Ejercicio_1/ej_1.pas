...
procedure realizarBajas(var Empleados: ArchivoEmpleados);
var
  nroABorrar: integer;
  emp, aux: empleado;
begin
  writeln('Ingrese el nro de empleado a borrar: ');
  readln(nroABorrar);
  reset(Empleados);
  // Me posiciono y guardo el ult registro
  seek(Empleados, fileSize(Empleados)-1);
  read(Empleados, aux);
  // Leo el primer registro y busco el reg a eliminar
  seek(Empleados, 0);
  read(Empleados, emp);
  while not (eof(Empleados)) and (emp.Nro <> nroABorrar) do begin
    read(Empleados, emp);
  end;
  if (emp.Nro = nroABorrar) then begin
    seek(Empleados, filepos(Empleados)-1);
    write(Empleados, aux);
    seek(Empleados, fileSize(Empleados)-1);
    truncate(Empleados);
...
Procedure abrirArchivo (var Empleados: ArchivoEmpleados);
  ...
  begin
    ...
    else if (opcion = 'h') then begin
      realizarBajas(Empleados);
    end
    else begin
